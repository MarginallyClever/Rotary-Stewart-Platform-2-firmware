...
implements PropertyChangeListener {
	...
	Model model_top = new Model();
	Model model_arm = new Model();
	Model model_base = new Model();
	
	class Arm {
		  Vector3f shoulder = new Vector3f();
		  Vector3f elbow = new Vector3f();
		  Vector3f shoulder_to_elbow = new Vector3f();
		  ...
		  public void set(Arm other) {
			  ...
			  shoulder_to_elbow.set(other.shoulder_to_elbow);
...
*/
	...
	void Init(GL2 gl2) {
		...
		model_top .Load(gl2, current+"/top.STL" );
		model_arm .Load(gl2, current+"/arm.STL" );
	...
	protected void IK_update_shoulder_angles(MotionState state) throws AssertionError {
		Vector3f ortho = new Vector3f(),w = new Vector3f(),wop = new Vector3f(),temp = new Vector3f(),r = new Vector3f();
		  ...
		  for(i=0;i<6;++i) {
		    ...
		    if( ( arm.shoulder_to_elbow.dot( temp ) ) < 0 ) x=-x;
	...
	public void FinalizeMove() {
		...
		if(arm_moved) {
			if(homed && follow_mode && this.ReadyForCommands() ) {
				...
				this.SendCommand("G0 X"+motion_now.finger_tip.x
...
//		    armb.elbow = n1*BASE_TO_SHOULDER_X + motion_future.base_up*BASE_TO_SHOULDER_Z + o1*(BASE_TO_SHOULDER_Y+BICEP_LENGTH);
		    ...
		    arma.shoulder_to_elbow.set(o1);
		    arma.shoulder_to_elbow.scale(-1);
		    armb.shoulder_to_elbow.set(o1);
		  }
	}
